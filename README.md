...
From there, you can make a local copy of one of the examples based on the kind of system setup you want to use. If you are unsure of which, we recommend using the Dual Axis Tracking Example for older MCUs and the Full System Example for modern MCUs. Older storage constrained MCUs (< 512kB Flash) may need further modifications (and possibly external hardware) so is recommended only for advanced users.
