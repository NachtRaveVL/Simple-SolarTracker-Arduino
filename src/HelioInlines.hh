...
struct I2CDeviceSetup {
    ...
    inline I2CDeviceSetup(TwoWire *i2cWire = HELIO_USE_WIRE, uint32_t i2cSpeed = 100000U, uint8_t i2cAddress = 0b000) : wire(i2cWire), speed(i2cSpeed), address(i2cAddress) { ; }
    inline I2CDeviceSetup(uint32_t i2cSpeed, uint8_t i2cAddress = 0b000, TwoWire *i2cWire = HELIO_USE_WIRE) : wire(i2cWire), speed(i2cSpeed), address(i2cAddress) { ; }
    inline I2CDeviceSetup(uint8_t i2cAddress, TwoWire *i2cWire = HELIO_USE_WIRE, uint32_t i2cSpeed = 100000U) : wire(i2cWire), speed(i2cSpeed), address(i2cAddress) { ; }
};
...
struct SPIDeviceSetup {
    ...
    inline SPIDeviceSetup(SPIClass *spiClass = HELIO_USE_SPI, uint32_t spiSpeed = F_SPD, pintype_t spiCS = -1) : spi(spiClass), speed(spiSpeed), cs(spiCS) { ; }
    inline SPIDeviceSetup(uint32_t spiSpeed, pintype_t spiCS = -1, SPIClass *spiClass = HELIO_USE_SPI) : spi(spiClass), speed(spiSpeed), cs(spiCS) { ; }
    inline SPIDeviceSetup(pintype_t spiCS, SPIClass *spiClass = HELIO_USE_SPI, uint32_t spiSpeed = F_SPD) : spi(spiClass), speed(spiSpeed), cs(spiCS) { ; }
};
...
struct Twilight {
    ...
    inline Twilight() : sunrise(HELIO_NIGHT_FINISH_HR), sunset(HELIO_NIGHT_START_HR), isUTC(false) { ; }
