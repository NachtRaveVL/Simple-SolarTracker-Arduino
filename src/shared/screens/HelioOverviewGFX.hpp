...
template <class T>
HelioOverviewGFX<T>::HelioOverviewGFX(HelioDisplayAdafruitGFX<T> *display, const void *clockFont, const void *detailFont)
      ...
      _skyBlue(255), _skyRed(0), _timeMag(1), _dateMag(1), _lastTime((uint32_t)0), _timeHeight(0), _dateHeight(0)
{
    ...
    for (int i = 0; i < HELIO_UI_STARFIELD_MAXSIZE; ++i) {
...
template <class T>
HelioOverviewGFX<T>::~HelioOverviewGFX()
{ ; }

template <class T>
void HelioOverviewGFX<T>::drawBackground(Coord pt, Coord sz, Pair<uint16_t, uint16_t> &screenSize)
{
...
template <class T>
void HelioOverviewGFX<T>::renderOverview(bool isLandscape, Pair<uint16_t, uint16_t> screenSize)
{
