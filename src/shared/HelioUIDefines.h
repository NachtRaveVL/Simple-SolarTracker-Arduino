...
#define HELIO_UI_SPRITE_MAXYSIZE        16                  // Sprite max Y (pixel height) - aka # rows for VRAM buffer, when enabled
...
#define HELIO_UI_I2C_OLED_BASEADDR      0x78                // Base address of I2C U8g2 OLEDs (bitwise or'ed with passed address, some devices may need 0x7e)
#define HELIO_UI_BACKLIGHT_TIMEOUT      5 * SECS_PER_MIN    // Backlight timeout, in seconds
#define HELIO_UI_START_AT_OVERVIEW      false               // UI starts at overview screen (true), else menu screen (false)
#define HELIO_UI_DEALLOC_AFTER_USE      !HAS_LARGE_SRAM     // If menu data should be unloaded after use (true = lower memory usage, less responsive transitions), or stay memory-resident (false = higher memory usage, more responsive transitions)

#define HELIO_UI_KEYREPEAT_SPEED        20                  // Default key press repeat speed, in ticks (lower = faster)
#define HELIO_UI_REMOTESERVER_PORT      3333                // Default remote control server's listening port
#define HELIO_UI_2X2MATRIX_KEYS         "#BA*"              // 2x2 matrix keyboard keys (R/S1,D/S2,U/S3,L/S4), forced PROGMEM chars
#define HELIO_UI_3X4MATRIX_KEYS         "123456789*0#"      // 3x4 matrix keyboard keys (123,456,789,*0#), forced PROGMEM chars
#define HELIO_UI_4X4MATRIX_KEYS         "123A456B789C*0#D"  // 4x4 matrix keyboard keys (123A,456B,789C,*0#D), forced PROGMEM chars
#define HELIO_UI_MATRIX_ACTIONS         "#*AB"              // Assigned enter/select char, delete/exit char, back char, and next char on keyboard, forced PROGMEM
#define HELIO_UI_TFTTOUCH_USES_RAW      false               // Raw touch usage for TFTTouch

#define HELIO_UI_GFX_USE_ANALOG_SLIDER  true                // Default analog slider usage for graphical displays displaying variable value ranges
#define HELIO_UI_GFX_USE_EDITING_ICONS  false               // Default editing icon usage for graphical displays
#define HELIO_UI_MENU_TITLE_MAG_LEVEL   2                   // Menu title font magnification level (int only)
#define HELIO_UI_MENU_ITEM_MAG_LEVEL    2                   // Menu item font magnification level (int only)
#define HELIO_UI_IOT_MONITOR_TEXT       "IoT Monitor"       // Menu IoT monitor item text, forced PROGMEM chars
#define HELIO_UI_AUTHENTICATOR_TEXT     "Authenticator"     // Menu authenticator item text, forced PROGMEM chars
#define HELIO_UI_SIMHUB_STATUS_MENU_ID  60                  // Simhub remote status boolean menu item ID
#define HELIO_UI_OVERVIEW_ACT_MENU_ID   7                   // Overview action menu item ID
...
enum Helio_DisplayRotation : signed char {
    ...
    Helio_DisplayRotation_HorzMirror,                       // Horizontally mirrored (iff supported, touchscreen tuning orientation pass-through w/o rotation)
    Helio_DisplayRotation_VertMirror,                       // Vertically mirrored (iff supported, touchscreen tuning orientation pass-through w/o rotation)
...
enum Helio_TouchscreenOrientation : signed char {
    Helio_TouchscreenOrientation_Same,                      // Apply same orientation as display rotation (converts display rotation to swapXY/invX/invY values)
    Helio_TouchscreenOrientation_Plus1,                     // Apply same orientation as display rotation + R1, %4->[R0,R3] (converts display rotation + 90° clockwise rotation to swapXY/invX/invY values)
    Helio_TouchscreenOrientation_Plus2,                     // Apply same orientation as display rotation + R2, %4->[R0,R3] (converts display rotation + 180° clockwise rotation to swapXY/invX/invY values)
    Helio_TouchscreenOrientation_Plus3,                     // Apply same orientation as display rotation + R3, %4->[R0,R3] (converts display rotation + 270° clockwise rotation to swapXY/invX/invY values)
...
enum Helio_TitleMode : signed char {
    Helio_TitleMode_None,                                   // Title not displayed
    Helio_TitleMode_FirstRow,                               // Title only appears when first row selected (if supported)
    Helio_TitleMode_Always,                                 // Title is always displayed

    Helio_TitleMode_Count,                                  // Placeholder
    Helio_TitleMode_Undefined = -1                          // Placeholder
};
...
enum Helio_ST77XXKind : signed char {
    Helio_ST7735Tag_B                   = (int8_t)0xff,     // ST7735B B model (128x160, 20480 pixels)
    Helio_ST7735Tag_Green               = (int8_t)0x00,     // ST7735S Green tag (1.8" TFT /w offset such as WaveShare, 128x160, 20480 pixels)
    Helio_ST7735Tag_Green18             = (int8_t)0x00,     // ST7735S 18Green tag (alias of Green, 128x160, 20480 pixels)
    Helio_ST7735Tag_Red                 = (int8_t)0x01,     // ST7735R Red tag (128x160, 20480 pixels)
    Helio_ST7735Tag_Red18               = (int8_t)0x01,     // ST7735R 18Red tag (alias of Red, 128x160, 20480 pixels)
    Helio_ST7735Tag_Black               = (int8_t)0x02,     // ST7735S Black tag (1.8" TFT, 128x160, 20480 pixels)
    Helio_ST7735Tag_Black18             = (int8_t)0x02,     // ST7735S 18Black tag (alias of Black, 128x160, 20480 pixels)
    Helio_ST7735Tag_Green144            = (int8_t)0x01,     // ST7735R 144Green tag (1.44" TFT, 128x128, 16384 pixels)
    Helio_ST7735Tag_Mini                = (int8_t)0x04,     // ST7735S Mini160x80 tag (0.96" TFT, 80x160, 12800 pixels - if inverted try MiniPlugin)
    Helio_ST7735Tag_MiniPlugin          = (int8_t)0x06,     // ST7735S Mini160x80_Plugin tag (0.96" TFT /w plug-in FPC, 80x160, 12800 pixels)
    Helio_ST7735Tag_HalloWing           = (int8_t)0x05,     // ST7735R HalloWing tag (upside-down 144Green, 128x128, 16384 pixels)
