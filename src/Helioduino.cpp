...
bool Helioduino::initFromEEPROM(bool jsonFormat)
{
    ...
    if (!_systemData) {
        ...
        if (getEEPROM() && _eepromBegan && _sysDataAddress != -1) {
            HelioEEPROMStream eepromStream(_sysDataAddress, getEEPROMDeviceSize() - _sysDataAddress);
    ...
    if (_sdSetup.cfgType == DeviceSetup::SPISetup) {
        ...
        if (_netSetup.cfgType == DeviceSetup::SPISetup) {
            ...
            if (began.find((uintptr_t)_netSetup.cfgAs.ttl.serial) == began.end() || _netSetup.cfgAs.ttl.baud < began[(uintptr_t)_netSetup.cfgAs.ttl.serial]) {
                _netSetup.cfgAs.ttl.serial->begin((began[(uintptr_t)_netSetup.cfgAs.ttl.serial] = _netSetup.cfgAs.ttl.baud), (uint16_t)HELIO_SYS_ATWIFI_SERIALMODE);
            }
