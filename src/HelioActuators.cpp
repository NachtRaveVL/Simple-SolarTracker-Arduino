...
void HelioVariableMotorActuator::handleActivation()
{
    ...
    if (_enabled) {
        ...
        // Sensorless motors otherwise appear to stop instantly in software even though the
        // real mechanism keeps moving. Apply the configured coast once when power is removed.
        if (!getPositionSensor() && _coastTimeMillis && helioDirectionForOffset(_signedIntensity)) {
            const float coastDistance = getCoastDistance(getDistanceUnits()) * fabsf(_signedIntensity);
